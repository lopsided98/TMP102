...
function(GENERATE_ARDUINO_LIBRARY INPUT_NAME)
    ...
    if(NOT ${INPUT_NO_AUTOLIBS})
        arduino_debug_msg("Sources for ${INPUT_NAME}: ${ALL_SRCS}")
    ...
    arduino_debug_msg("${INPUT_NAME} libraries: ${ALL_LIBS}")
    target_include_directories(${INPUT_NAME} PUBLIC ${TARGET_LIBS})
...
function(GENERATE_ARDUINO_FIRMWARE INPUT_NAME)
    ...
    foreach(LIB_DEP ${TARGET_LIBS})
        ...
        list(APPEND LIB_DEP_INCLUDES ${LIB_DEP} "${LIB_DEP}/src")
    endforeach()

    if(NOT INPUT_NO_AUTOLIBS)
        setup_arduino_libraries(ALL_LIBS  ${INPUT_BOARD} "${ALL_SRCS}" "${INPUT_ARDLIBS}" "" "")
        foreach(LIB_INCLUDES ${ALL_LIBS_INCLUDES})
            arduino_debug_msg("Arduino Library Includes: ${LIB_INCLUDES}")
            list(APPEND LIB_DEP_INCLUDES ${LIB_INCLUDES})
        endforeach()
    endif()
    ...
    arduino_debug_msg("Firmware include directories: ${TARGET_LIBS}")
    target_include_directories(${INPUT_NAME} PUBLIC ${TARGET_LIBS})
...
function(setup_arduino_core VAR_NAME BOARD_ID)
    ...
    if(BOARD_CORE)
        if(NOT TARGET ${CORE_LIB_NAME})
            ...
            target_include_directories(${CORE_LIB_NAME} PUBLIC ${BOARD_CORE_PATH})
...
function(setup_arduino_library VAR_NAME BOARD_ID LIB_PATH COMPILE_FLAGS LINK_FLAGS)
    ...
    if(NOT TARGET ${TARGET_LIB_NAME})
        ...
        if(LIB_SRCS)
            ...
            arduino_debug_msg("${LIB_NAME} sources: ${LIB_SRCS}")
            ...
            set_target_properties(${TARGET_LIB_NAME} PROPERTIES
                COMPILE_FLAGS "${ARDUINO_COMPILE_FLAGS} ${COMPILE_FLAGS}"
                LINK_FLAGS "${ARDUINO_LINK_FLAGS} ${LINK_FLAGS}")
            list(APPEND LIB_INCLUDES "${LIB_PATH}" "${LIB_PATH}/src" "${LIB_PATH}/utility")
...
function(find_sources VAR_NAME LIB_PATH RECURSE)
    ...
    arduino_debug_msg("Find sources: ${LIB_PATH} ${RECURSE}")
